...
function cl_get_alt_account_ids($user_id = 0) {
    ...
    $settings = json($user_data['settings']);

    if (empty($settings) || !is_array($settings) || empty($settings['alt_accounts'])) {
        return array();
    }

    $alt_ids = array();

    if (is_array($settings['alt_accounts'])) {
        foreach ($settings['alt_accounts'] as $alt) {
            if (isset($alt['id']) && is_numeric($alt['id'])) {
                $alt_ids[] = (int) $alt['id'];
            }
        }
    }
...
function cl_get_alt_accounts($user_id = 0) {
    ...
    foreach ($alt_ids as $alt_id) {
        ...
        if (not_empty($alt_user)) {
            // Add full user data for display purposes
...
function cl_sync_alt_account_group($user_ids = array()) {
    ...
    // Sync alt accounts across all linked accounts' settings
    // Each user should have all other users in their alt_accounts list
    $alt_account_list = array();

    // Build the common alt accounts list
    ...
    // Update each user's settings with the full alt accounts list
    foreach ($user_ids as $user_id) {
        $user_data = cl_raw_user_data($user_id);
        if (empty($user_data)) {
            continue;
        ...
        // Build the alt accounts list for this user (exclude the user themselves)
        $alt_accounts = array();
        foreach ($alt_account_list as $alt) {
            if ((int) $alt['id'] !== (int) $user_id) {
                $alt_accounts[] = $alt;
            }
        }

        $settings['alt_accounts'] = $alt_accounts;
...
function cl_remove_alt_account_link($user_id = 0, $alt_id = 0) {
    ...
    // Get the current alt account list for this user
    $user_data = cl_raw_user_data($user_id);
    if (empty($user_data)) {
        return false;
    }
    ...
    $settings = json($user_data['settings']);
    if (!is_array($settings) || empty($settings['alt_accounts'])) {
        return false;
    }

    // Get all alt IDs for this user
    $alt_ids = cl_get_alt_account_ids($user_id);

    // Remove the alt_id from this user's list
    $new_alt_accounts = array();
    foreach ($settings['alt_accounts'] as $alt) {
        if ((int) $alt['id'] !== $alt_id) {
            $new_alt_accounts[] = $alt;
        }
    }

    // Also need to remove user_id from alt_id's alt list
    ...
    if (!empty($alt_user_data)) {
        ...
        $alt_new_accounts = array();
        if (!empty($alt_settings['alt_accounts']) && is_array($alt_settings['alt_accounts'])) {
            foreach ($alt_settings['alt_accounts'] as $acc) {
                if ((int) $acc['id'] !== $user_id) {
                    $alt_new_accounts[] = $acc;
                }
            }
        }

        $alt_settings['alt_accounts'] = $alt_new_accounts;
        cl_update_user_data($alt_id, array(
            'settings' => json($alt_settings, true)
        ));
    }

    // Update current user's alt accounts
    $settings['alt_accounts'] = $new_alt_accounts;
    cl_update_user_data($user_id, array(
        'settings' => json($settings, true)
    ));

    // If removing this link breaks the group (only 1 left), clear all alt accounts
    $remaining_alts = count($new_alt_accounts);
    if ($remaining_alts === 0) {
        // All alts removed from this user
        return true;
    }

    // For remaining alts, we need to check if they should still be linked
    // If alt_id had other alts, we need to rebuild their groups
    if (!empty($alt_ids) && count($alt_ids) > 1) {
        // Rebuild the group for remaining alts (excluding alt_id)
        $remaining_ids = array();
        ...
        if (count($remaining_ids) > 1) {
            cl_sync_alt_account_group($remaining_ids);
        }
    }
...
function cl_follow_decrease($follower_id = false, $following_id = false){
    ...
    if (not_empty($follower_udata)) {
        ...
        $follow_num = ($follower_udata['following'] -= 1);
        $follow_num = (empty($follow_num) || $follow_num < 0) ? 0 : $follow_num;
...
function cl_get_admin_sections() {
    $glob_data = glob(cl_full_path("apps/native/http/cpanel/sections/*.php"));
    $admin_sections = array();

    foreach($glob_data as $path_data) {
        $path_info = pathinfo($path_data);
        $file_name = $path_info["filename"];

        array_push($admin_sections, $file_name);
    }
...
function cl_get_admin_perms($user_id = false) {
    global $db;
    
    $perms_data = $db->where("user_id", $user_id)->getOne(T_ADMIN_PERMS);

    $admin_permissions = array();

    if (cl_queryset($perms_data)) {
        $admin_permissions = json($perms_data["admin_permissions"]);
    }
