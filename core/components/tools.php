...
function cl_get_media($media = '', $is_upload = false){
    ...
    elseif ($config['idrive3_storage'] == 'on') {
        $media_url  = cl_strf("https://%s.%s/%s", $config['idrive3_bucket_name'], $config['idrive3_endpoint_url'], $media);
        return $media_url;
    }
    else {
...
function cl_upload($data = array()) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        ...
        if (in_array($file_ext, array('gif','png','jpeg','jpg', "webp")) == true) {
            try {
                if ($file_ext != 'gif' && $file_ext != 'png') {
                    if ($cl["config"]["media_optimization_status"] == "on") {
                        require_once(cl_full_path("core/libs/Intervention/vendor/autoload.php"));


                        $optimized_img = \Intervention\Image\ImageManagerStatic::make($filename);

                        $optimized_img->save($filename, $cl["config"]["post_images_compression"]);
                    }
                }
            }

            catch (Exception $e) {
                cl_log_system_error($e->getMessage());
            }
        ...
        if ($cl['config']['as3_storage'] == 'on') {
            ...
            if ($aws_uploadfile == "Y") {
                try {
                    cl_upload2idrive_s3($filename);

                    if (not_empty($result['cropped'])) {
                        cl_upload2idrive_s3($result['cropped']);
                    }
                } catch (Exception $e) { /* pass */ }
            }
        }
...
function cl_upload2idrive_s3($filename = null, $del_localfile = "Y") {
    global $cl;

    if ($cl['config']['idrive3_storage'] == 'off') {
        return false;
    }
    else {
        if (empty($cl['config']['idrive3_api_key'])) {
            return false;
        }

        else if(empty($cl['config']['idrive3_api_secret_key'])) {
            return false;
        }

        else if(empty($cl['config']['idrive3_bucket_region'])) {
            return false;
        }

        else if(empty($cl['config']['idrive3_endpoint_url'])) {
            return false;
        }

        else if(empty($cl['config']['idrive3_bucket_name'])) {
            return false;
        }
            ...
            try {
                ...
                $amazon_s3        = new \Aws\S3\S3Client(array(
                    'version'     => 'latest',
                    'endpoint' => 'https://'.$cl['config']['idrive3_endpoint_url'],
                    'region'      => $cl['config']['idrive3_bucket_region'],
                    'credentials' => array(
                        'key'     => $cl['config']['idrive3_api_key'],
                        'secret'  => $cl['config']['idrive3_api_secret_key']
                    )
                ));

                $file_res = fopen($filename, 'r+');
                $up_aws_object     = $amazon_s3->putObject(array(
                    'Bucket'       => $cl['config']['idrive3_bucket_name'],
                    'Key'          => $filename,
                    'Body'         => $file_res,
                    'ACL'          => 'public-read',
                    'CacheControl' => 'max-age=3153600'
                ...
                if ($del_localfile == "Y") {
                    if ($amazon_s3->doesObjectExist($cl['config']['idrive3_bucket_name'], $filename)) {
                        cl_delete_loc_media($filename);
                    }
                }
...
function cl_delete_from_idrive_s3($filename = null) {
    global $cl;
    ...
    if ($cl['config']['idrive3_storage'] == 'off') {
        ...
        if (empty($cl['config']['idrive3_api_key'])) {
            ...
            try {
                ...
                if ($amazon_s3->doesObjectExist($cl['config']['idrive3_bucket_name'], $filename) != true) {
                    return true;
                }
...
function cl_create_user_avatar() {
    global $cl;

    $image_path = cl_strf("upload/default/faces/avatar-%d.png", rand(1, 55));

    return $image_path;
}

function cl_log_system_error($message = "") {
    global $cl;

    try {
        $log_file_path = cl_strf("core/logs/System-logs-%s.log", date("d-m-Y"));
        $file_src = fopen($log_file_path, "a");

        if (not_empty($cl["is_logged"])) {
            $message = "#[Username: " . $cl["me"]["name"] . "] Datetime " . date("d F, Y - h:i") . "; Error text: " . $message;
        }

        else{
            $message = "#[Anonymous user] Datetime " . date("d F, Y - h:i") . "; Error text: " . $message;
        }

        fwrite($file_src, $message . "\n\n");
