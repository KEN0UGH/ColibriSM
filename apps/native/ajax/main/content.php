...
if ($action == 'upload_post_image' && $cl["config"]["post_images_system"] == "on") {
            ...
            if (not_empty($post_data) && $post_data["type"] == "image") {
                if (empty($post_data['media']) || count($post_data['media']) < 10) {
                    ...
                    if (not_empty($file_upload['filename'])) {
                        ...
                        if ($cl["config"]["google_ai_vision_status"] == "on") {
                            $is_unsafe = cl_detect_unsafe_image($file_upload['filename']);
...
else if ($action == 'upload_post_video' && $cl["config"]["post_videos_system"] == "on") {
    if (empty($cl["is_logged"])) {
        $data['status'] = 400;
        $data['error']  = 'Invalid access token';
    }
                        ...
                        if (empty($upload_fail)) {
                            ...
                            if (is_posnum($img_id)) {
                                ...
                                if ($cl['config']['as3_storage'] == 'on') {
                                    ...
                                    cl_upload2idrive_s3($file_upload['filename']);

                                    cl_upload2idrive_s3($thumb_path);
                                }
...
else if ($action == 'upload_post_arecord' && $cl["config"]["post_record_system"] == "on") {
    if (empty($cl["is_logged"])) {
        $data['status'] = 400;
        $data['error']  = 'Invalid access token';
    }
...
else if ($action == 'upload_post_music' && $cl["config"]["post_audio_system"] == "on") {
    if (empty($cl["is_logged"])) {
        $data['status'] = 400;
        $data['error']  = 'Invalid access token';
    }
...
else if ($action == 'upload_post_document' && $cl["config"]["post_documents_system"] == "on") {
    if (empty($cl["is_logged"])) {
        $data['status'] = 400;
        $data['error']  = 'Invalid access token';
    }
    ...
    if (empty($cl["is_logged"])) {
        ...
        $donation_amount  = fetch_or_get($_POST['donation_amount'], "");
        ...
        if (not_empty($post_data) && not_empty($post_data["media"])) {
            ...
            if(not_empty($post_text) && not_empty($donation_amount) && is_numeric($donation_amount) && $donation_amount > 0) {
                if ($cl["config"]["donation_system_status"] == "on") {
                    $post_data_update['is_donation_post'] = "Y";
                    $post_data_update['donation_amount'] = $donation_amount;

                    $post_text_array = explode("\n", $post_text, 2);

                    if (is_array($post_text_array) && count($post_text_array) == 2) {
                        $post_text_title = $post_text_array[0];
                        $post_text_desc = $post_text_array[1];

                        if (len_between($post_text_title, 32, 120) && len($post_text_desc) > 120) {
                            $post_data_update["title"] = cl_text_secure($post_text_title);
                            $post_data_update["text"] = cl_text_secure($post_text_desc);
                        }
                    }
                }
            }

            cl_update_post_data($post_id, $post_data_update);
            ...
            if (not_empty($post_text) || not_empty($gif_src) || not_empty($og_data) || not_empty($poll_data)) {
                ...
                if(not_empty($post_text) && not_empty($donation_amount) && is_numeric($donation_amount) && $donation_amount > 0) {
                    if ($cl["config"]["donation_system_status"] == "on") {
                        $insert_data['is_donation_post'] = "Y";
                        $insert_data['donation_amount'] = $donation_amount;
                        ...
                        if (is_array($post_text_array) && count($post_text_array) == 2) {
                            ...
                            if (len_between($post_text_title, 32, 120) && len($post_text_desc) > 120) {
                                $insert_data["title"] = cl_text_secure($post_text_title);
                                $insert_data["text"] = cl_text_secure($post_text_desc);
                            }
                        }
                    }
                }
                ...
                if(not_empty($post_text) && not_empty($poll_data) && cl_is_valid_poll($poll_data)) {
                    if ($cl["config"]["post_polls_system"] == "on") {
                        $insert_data['og_data']   = "";
                        $gif_src                  = "";
                ...
                if (cl_is_following($me['id'], $user_id)) {
                    ...
                    if ($udata["follow_privacy"] == "everyone") {
                        ...
                        if (cl_follow_requested($me['id'], $user_id)) {
                            ...
                            $follow_request_id = cl_follow_request($me['id'], $user_id);
                            ...
                            if ($follow_request_id) {
                                ...
                                cl_notify_user(array(
                                    ...
                                    'user_id'  => $user_id,
                                    'entry_id' => $me["id"],
                                    'json' => cl_minify_js(json(array("follow_request_id" => $follow_request_id), true))
                                ));
                            }
                        }
                    }
    ...
    if (is_posnum($post_id)) {
        ...
        if (not_empty($post_data)) {
            $views_count = ($post_data['views_count'] += 1);
            $data['status']      = 200;
...
else if($action == 'post_access_toggle') {
    ...
    if (empty($cl["is_logged"])) {
        ...
        if (is_posnum($post_id)) {
            ...
            if (not_empty($post_data) && $post_data["user_id"] == $me["id"]) {
                if ($post_data["is_free_post"] == "N") {
                    cl_update_post_data($post_id, array(
                        "is_free_post" => "Y"
                    ));
...
else if($action == 'stop_poll' && not_empty($cl["is_logged"])) {
    $data['err_code'] = 0;
    $data['status']   = 400;
    $post_id          = fetch_or_get($_POST['id'], 0);
...
else if($action == "update_web_device_id") {
    $data['err_code'] = 0;
    $data['status']   = 400;

    $web_device_id = fetch_or_get($_POST["id"], "");

    if (not_empty($cl["is_logged"])) {
        if (is_string($web_device_id) && len_between($web_device_id, 25, 100)) {
...
else if($action == "delete_web_device_id") {
    $data['err_code'] = 0;
    $data['status']   = 400;
